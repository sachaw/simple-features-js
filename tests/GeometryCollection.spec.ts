...
test.group("GeometrySerializableTest", () => {
  ...
  test("test multi line string", ({ expect }) => {
    const lineStrings = [];
    for (let i = 0; i < 5; i++) {
  ...
  test("test multi polygon", ({ expect }) => {
    const polygons = [];
    for (let i = 0; i < 5; i++) {
  ...
  test("test multi surface", ({ expect }) => {
    const surfaces = [];
    for (let i = 0; i < 5; i++) {
      if (i % 2 === 0) {

...
function geometryCentroidTester(geometry: Geometry, expect: Expect) {
  ...
  if (childTypes.length === 0) {
    ...
    for (const childType of childTypes) {
      ...
      if (childChildHierarchy) {
        testChildHierarchy(childType, childChildHierarchy, expect);
      }
    }
  }
}

test.group("GeometryUtilsTest", () => {
  test("test point centroid", ({ expect }) => {
    for (let i = 0; i < GEOMETRIES_PER_TEST; i++) {
      ...
      geometryCentroidTester(point, expect);
    }
  });

  test("test line string centroid", ({ expect }) => {
    for (let i = 0; i < GEOMETRIES_PER_TEST; i++) {
      ...
      geometryCentroidTester(lineString, expect);
    }
  });

  test("test polygon centroid", ({ expect }) => {
    for (let i = 0; i < GEOMETRIES_PER_TEST; i++) {
      // Create and test a polygon
      const polygon = createPolygon();
      expect(GeometryUtils.getDimension(polygon)).toEqual(2);
      geometryCentroidTester(polygon, expect);
    }
  });

  test("test MultiPoint centroid", ({ expect }) => {
    for (let i = 0; i < GEOMETRIES_PER_TEST; i++) {
      ...
      geometryCentroidTester(multiPoint, expect);
    }
  });

  test("test MultiLineString centroid", ({ expect }) => {
    for (let i = 0; i < GEOMETRIES_PER_TEST; i++) {
      ...
      geometryCentroidTester(multiLineString, expect);
    }
  });

  test("test MultiPolygon centroid", ({ expect }) => {
    for (let i = 0; i < GEOMETRIES_PER_TEST; i++) {
      ...
      geometryCentroidTester(multiPolygon, expect);
    }
  });

  test("test GeometryCollection centroid", ({ expect }) => {
    for (let i = 0; i < GEOMETRIES_PER_TEST; i++) {
      ...
      geometryCentroidTester(geometryCollection, expect);
    }
  });

  test("test polygon centroid with and without hole", ({ expect }) => {
    const polygon = Polygon.create();
    const lineString = LineString.create();
    ...
    polygon.addRing(lineString);
    expect(GeometryUtils.getDimension(polygon)).toEqual(2);
    let centroid = geometryCentroidTester(polygon, expect);
    ...
    polygon.addRing(holeLineString);
    expect(GeometryUtils.getDimension(polygon)).toEqual(2);
    centroid = geometryCentroidTester(polygon, expect);
    expect(centroid.x).toEqual(-15.0);
    expect(centroid.y).toEqual(-7.5);
  });

  test("test copy, minimize, and normalize", ({ expect }) => {
    const polygon = Polygon.create();
  ...
  test("test simplify points", ({ expect }) => {
  ...
  test("test point in polygon", ({ expect }) => {
    const points = [];
  ...
  test("test closed polygon", ({ expect }) => {
    const points = [];
  ...
  test("test point on line", ({ expect }) => {
    const points = [];
